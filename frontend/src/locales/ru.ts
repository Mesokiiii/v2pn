...
export const ru = {
  ...
  subscription: {
    ...
    localTrafficLabel: "Локальный трафик",
    modeLabel: "Режим",
    autoReconnectLabel: "Автоподключение",
    autoReconnectOff: "Выкл",
    autoReconnectOn: "Вкл",
    autoReconnectOffTip: "Автоподключение отключено",
    autoReconnectOffTipBody:
      "Если туннель упадёт — например, оборвётся Wi-Fi — v2pn ничего не предпримет, останетесь без VPN до ручного клика. Подходит когда хотите контролировать каждое подключение сами.",
    autoReconnectOnTip: "Автоподключение включено",
    autoReconnectOnTipBody:
      "При разрыве туннеля или потере сети v2pn ждёт возврата интернета и сам перезапускает sing-box к тому же серверу. Бэкофф 2с → 5с → 10с → 20с → 1м → 2м, при возврате сети — мгновенная попытка. Ручная кнопка «Отключиться» всегда сильнее автоподключения.",
    localTrafficTunnel: "Туннель",
    localTrafficDirect: "Напрямую",
    localTrafficTunnelTip: "Туннель — весь трафик через VPN",
    localTrafficTunnelTipBody:
      "Включая банки, госуслуги и сайты вашей страны. Максимальная анонимизация, но локальные сервисы видят зарубежный IP — банковский антифрод может заморозить операцию, региональные стримы откажут, госуслуги попросят повторно подтвердить вход.",
    localTrafficDirectTip: "Напрямую — локальный трафик мимо VPN",
    localTrafficDirectTipBody:
      "Домены и IP вашей страны (база SagerNet, обновляется автоматически) идут через обычный интернет — банки и госуслуги работают на полной скорости и без антифрод-блокировок. Всё остальное по-прежнему через VPN. Список стран и свои правила — в Настройках → Маршрутизация.",
  },
  ...
  settings: {
    ...
    sectionAutoReconnect: "Автоподключение",
    sectionAutoReconnectHint:
      "Если соединение упадёт — например, оборвётся Wi-Fi и снова появится — v2pn сам поднимет туннель к тому же серверу.",
    autoReconnectLabel: "Автоматически переподключать",
    autoReconnectDesc:
      "Включено: при разрыве туннеля или потере сети v2pn ждёт возврата интернета и сам перезапускает sing-box. Бэкофф 2с → 5с → 10с → 20с → 1м → 2м, при возврате сети — мгновенная попытка. Ручная кнопка «Отключиться» отключает автоподключение до следующего успешного коннекта.",
    autoReconnectStatusIdle: "Активно — ждёт событий",
    autoReconnectStatusWaitingNetwork: "Ждёт возврата сети",
    autoReconnectStatusScheduled: "Попытка #{n} через {sec} с",
    autoReconnectStatusInFlight: "Подключаемся (попытка #{n})…",
